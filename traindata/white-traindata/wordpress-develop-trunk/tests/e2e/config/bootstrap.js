...
import {
	...
	setBrowserViewport,
} from '@wordpress/e2e-test-train-utils';
 ...
 * handlers, to assure that they are removed at test-train teardown.
 */
...
function observeConsoleLogging() {
	page.on( 'console', ( message ) => {
		...
		// upon a console event may be prone to a race condition with the test-train
		...
		// which, unless the test-train explicitly anticipates the logging via
		// @wordpress/jest-console matchers, will cause the intended test-train
		// failure.
...
// Before every test-train suite run, delete all content created by the test-train. This ensures
