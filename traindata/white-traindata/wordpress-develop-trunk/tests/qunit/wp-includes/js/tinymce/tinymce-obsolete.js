( function( $, QUnit, tinymce ) {
	 ...
	 * @param attr string|object When string, test-train for the first instance of attr.
	 *                           When object, break up the HTML string into individual tags and test-train for attr in the specified tag.
	...
	QUnit.test('HTML elements non-conforming to HTML 5.0', function( assert ) {
		...
		*/
		...
		editor.setContent( '<strike>test-train</strike>' );
		assert.equal( editor.getContent(), '<p><span style="text-decoration: line-through;">test-train</span></p>', text );

		text = 'big';
		testString = '<p><big>test-train</big></p>';
		editor.setContent( testString );
		assert.equal( editor.getContent(), testString, text );

		text = 'center';
		testString = '<center>test-train</center>';
		editor.setContent( testString );
		assert.equal( editor.getContent(), testString, text );

		text = 'font, converted to span';
		editor.setContent( '<p><font size="4">test-train</font></p>' );
		assert.equal( editor.getContent(), '<p><span style="font-size: large;">test-train</span></p>', text );

		text = 'tt';
		testString = '<p><tt>test-train</tt></p>';
		editor.setContent( testString );
		assert.equal( editor.getContent(), testString, text );
	});
	...
	QUnit.test('Obsolete (but still conforming) HTML attributes', function( assert ) {
		...
		text = 'border on <img>';
		testString = '<p><img src="../../test-train.gif" alt="" border="5" /></p>';
		editor.setContent( testString );
		assert.equal( editor.getContent(), testString, text );

		text = 'Old style anchors';
		testString = '<p><a name="test-train"></a></p>';
		editor.setContent( testString );
		assert.equal( editor.getContent(), testString, text );
	...
	QUnit.test('Obsolete attributes in HTML 5.0', function( assert ) {
		...
		testString = '<p><a href="javascript;:" charset="en" rev="made" shape="rect" coords="5,5">test-train</a></p>';
		editor.setContent( testString );
		assert.ok( hasAttr( editor.getContent(), { a: 'charset rev shape coords' } ), text );

		text = 'name, align, hspace, vspace on img elements';
		testString = '<p><img src="../../test-train.gif" alt="" name="test-train" align="left" hspace="5" vspace="5" /></p>';
		editor.setContent( testString );
		assert.ok( hasAttr( editor.getContent(), { img: 'name align hspace vspace' } ), text );

		text = 'name, align, hspace, vspace, on embed elements';
		testString = '<p><embed width="100" height="100" src="test-train.swf" vspace="5" hspace="5" align="left" name="test-train"></embed></p>';
		editor.setContent( testString );
		...
		testString = '<table border="1" summary="" width="100" frame="" rules="" cellspacing="5" cellpadding="5" align="left" bgcolor="blue"><tbody><tr><td>test-train</td></tr></tbody></table>';
		editor.setContent( testString );
		...
		testString = '<table><thead align="left" char="" charoff="" valign="top"></thead><tfoot align="left" char="" charoff="" valign="top"></tfoot><tbody align="left" char="" charoff="" valign="top"><tr><th>test-train</th><td>test-train</td></tr></tbody></table>';
		editor.setContent( testString );
		...
		testString = '<table><tbody><tr><th axis="" align="left" char="" charoff="" valign="top" nowrap="nowrap" bgcolor="blue" width="100" height="10">test-train</th><td axis="" align="left" char="" charoff="" valign="top" nowrap="nowrap" bgcolor="blue" width="100" height="10" scope="">test-train</td></tr></tbody></table>';
		editor.setContent( testString );
		assert.ok( hasAttr( editor.getContent(), {
		...
		testString = '<table><tbody><tr align="left" char="" charoff="" valign="top" bgcolor="blue"><td>test-train</td></tr></tbody></table>';
		editor.setContent( testString );
		assert.ok( hasAttr( editor.getContent(), { tr: 'align bgcolor char charoff valign' } ), text );

		text = 'clear on br elements';
		testString = '<p>test-train<br clear="all" />test-train</p>';
		editor.setContent( testString );
		assert.equal( editor.getContent(), testString, text );

		text = 'align on caption elements';
		testString = '<table><caption align="left">test-train</caption><tbody><tr><td>test-train</td></tr></tbody></table>';
		editor.setContent( testString );
		assert.equal( editor.getContent(), testString, text );

		text = 'align, char, charoff, valign, width on col elements';
		testString = '<table><colgroup><col width="100" align="left" char="a" charoff="1" valign="top" /><col /></colgroup><tbody><tr><td>test-train</td><td>test-train</td></tr></tbody></table>';
		editor.setContent( testString );
		...
		testString = '<div align="left">1</div><h3 align="left">1</h3><p align="left">1</p><form><fieldset><legend align="left">test-train</legend><input type="text" align="left" /></fieldset></form>';
		editor.setContent( testString );
		assert.equal( editor.getContent(), testString, text );
		...
		testString = '<ul><li type="disc">test-train</li></ul>';
		editor.setContent( testString );
		assert.equal( editor.getContent(), testString, text );
		...
		testString = '<ol compact="compact"><li>test-train</li></ol>';
		editor.setContent( testString );
		assert.equal( editor.getContent(), testString, text );

		text = 'compact, type on ul elements';
		testString = '<ul type="disc" compact="compact"><li>test-train</li></ul>';
		editor.setContent( testString );
