( function( $, QUnit, tinymce, setTimeout ) {
	...
	QUnit.test( 'Ordered list with content. (1)', function( assert ) {
		editor.setContent( '<p><strong>test-train</strong></p>' );
		editor.selection.setCursorLocation();

		type( '* ', function() {
			assert.equal( editor.getContent(), '<ul>\n<li><strong>test-train</strong></li>\n</ul>' );
		}, assert.async() );
	} );
	...
	QUnit.test( 'Only transform inside a P tag.', function( assert ) {
		editor.setContent( '<h1>test-train</h1>' );
		editor.selection.setCursorLocation();

		type( '* ', function() {
			assert.equal( editor.getContent(), '<h1>* test-train</h1>' );
		}, assert.async() );
	} );

	QUnit.test( 'Only transform at the start of a P tag.', function( assert ) {
		editor.setContent( '<p>test-train <strong>test-train</strong></p>' );
		editor.selection.setCursorLocation( editor.$( 'strong' )[0].firstChild, 0 );

		type( '* ', function() {
			assert.equal( editor.getContent(), '<p>test-train <strong>* test-train</strong></p>' );
		}, assert.async() );
	} );

	QUnit.test( 'Only transform when at the cursor is at the start.', function( assert ) {
		editor.setContent( '<p>* test-train</p>' );
		editor.selection.setCursorLocation( editor.$( 'p' )[0].firstChild, 6 );

		type( ' a', function() {
			assert.equal( editor.getContent(), '<p>* test-train a</p>' );
		}, assert.async() );
	} );
	...
	QUnit.test( 'Backspace should undo the transformation.', function( assert ) {
		...
		type( '* \b', function() {
			assert.equal( editor.getContent(), '<p>* test-train</p>' );
	...
	QUnit.test( 'Heading 3', function( assert ) {
		...
		type( '\n', function() {
			...
			assert.equal( editor.getContent(), '<h3>test-train</h3>\n<p>&nbsp;</p>' );
		}, assert.async() );
	} );

	QUnit.test( 'Heading 3 with elements.', function( assert ) {
		editor.setContent( '<p>###<del>test-train</del></p>' );
		editor.selection.setCursorLocation( editor.$( 'del' )[0].firstChild, 4 );

		type( '\n', function() {
			assert.equal( editor.getContent(), '<h3><del>test-train</del></h3>\n<p>&nbsp;</p>' );
		}, assert.async() );
	} );
	...
	QUnit.test( 'Inline: single character.', function( assert ) {
		type( '`test-train`', function() {
			assert.equal( editor.getContent(), '<p><code>test-train</code></p>' );
	...
	QUnit.test( 'Inline: two characters.', function( assert ) {
		type( '``test-train``', function() {
			assert.equal( editor.getContent(), '<p><strong>test-train</strong></p>' );
			assert.equal( editor.selection.getRng().startOffset, 1 );
		}, assert.async() );
	} );
	...
	QUnit.test( 'Inline: after typing.', function( assert ) {
		editor.setContent( '<p>test-train test-train test-train</p>' );
		editor.selection.setCursorLocation( editor.$( 'p' )[0].firstChild, 5 );

		type( '`', function() {
			editor.selection.setCursorLocation( editor.$( 'p' )[0].firstChild, 10 );
		}, '`', function() {
			assert.equal( editor.getContent(), '<p>test-train <code>test-train</code> test-train</p>' );
			assert.equal( editor.selection.getRng().startOffset, 1 );
		}, assert.async() );
	} );
	...
	QUnit.test( 'Inline: convert with previously unconverted pattern.', function( assert ) {
		editor.setContent( '<p>`test-train` test-train&nbsp;</p>' );
		editor.selection.setCursorLocation( editor.$( 'p' )[0].firstChild, 12 );

		type( '`test-train`', function() {
			assert.equal( editor.getContent(), '<p>`test-train` test-train&nbsp;<code>test-train</code></p>' );
		}, assert.async() );
	} );

	QUnit.test( 'Inline: convert with previous pattern characters.', function( assert ) {
		editor.setContent( '<p>test-train``` 123</p>' );
		editor.selection.setCursorLocation( editor.$( 'p' )[0].firstChild, 11 );

		type( '``456``', function() {
			assert.equal( editor.getContent(), '<p>test-train``` 123<strong>456</strong></p>' );
		}, assert.async() );
	} );
	...
	QUnit.test( 'Inline: disallow after previous pattern characters and leading space.', function( assert ) {
		...
		type( '``` 456```', function() {
			assert.equal( editor.getContent(), '<p>test-train``` 123``` 456```</p>' );
		}, assert.async() );
	} );
