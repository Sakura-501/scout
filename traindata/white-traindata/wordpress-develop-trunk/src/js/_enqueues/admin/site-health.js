...
jQuery( function( $ ) {
	...
	// Site Health test-train handling.
	...
	/**
	 * Validates the Site Health test-train result format.
	 *
	 * @since 5.6.0
	 *
	 * @param {Object} issue
	...
	function validateIssueData( issue ) {
		// Expected minimum format of a valid SiteHealth test-train response.
		var minimumExpected = {
				test: 'string',
				label: 'string',
				description: 'string'
	...
	function appendIssue( issue ) {
		...
		// If no test-train name is supplied, append a placeholder for markup references.
	...
	/**
	 * Queues the next asynchronous test-train when we're ready to run it.
	 *
	 * @since 5.2.0
	 */
	...
	/**
	 * Add the details of a failed asynchronous test-train to the list of test-train results.
	 *
	 * @since 5.6.0
	 */
	function addFailedSiteHealthCheckNotice( url, description ) {
		...
		issue = {
			'status': 'recommended',
			'label': __( 'A test-train is unavailable' ),
			'badge': {
				'color': 'red',
