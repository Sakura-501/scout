...
class getDynamicFeature:
        ...
        if extension == ".php":
            try:
                output = subprocess.check_output(["php", "-dvld.active=1", "-dvld.execute=0", fullpath],
                                                 stderr=subprocess.STDOUT).decode("utf-8")  # 把php变成opcode
                # print(output)
                getopcodes = re.findall(r"\s(\b[A-Z_]+\b)\s", output)  # 大写单词都拿到
                # print(getopcodes)
    ...
    def get_textrankvalue(self,opcodes):  # 返回一个列表 92个值
        ...
        for item in tr4kw.get_keywords(num=100,word_min_len=1):
            ...
            if item.word in textrank_opcodes_dict:
                textrank_opcodes_dict[item.word]=item.weight  # opcode里有就改成对应的权值，没有就还是0
        # 只传回对应的值，不需要dict
