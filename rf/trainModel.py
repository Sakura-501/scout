...
allrealopcodes = []
filelist = {}  # 使用fileread之后就会变成‘’文件名“：”绝对路径“
begin_time = time.time()
# 预训练数据
pre_feature_value = []  # 训练集n个php * 98
label = []  # n*1
black_number = 0
white_number = 0

# 特征值提取后，需要使用pickle序列化保存成文件，以供后续读取训练，这是保存路径
prefeature_pickleload_name = r"traindata/pre_feature-default.pkl"  # 加r可以防止字符转义
...
class trainModel:
    ...
    def create_model(self):
        ...
        rfc = RandomForestClassifier(bootstrap=True, n_estimators=100, criterion="gini", min_samples_split=2,
    ...
    def get_all_predata(self):
        """
        得到n*98的特征和n*1的label，放到pre_pickle类中，并用pickle序列化存储
        """
        global pre_feature_value, label, black_number, white_number
    ...
    def pikle_dump(self):
        ...
        print(label)
        print(len(label))
        print("black: " + str(black_number) + " ,white: " + str(white_number))
...
#     trainModel1 = trainModel()
#     a, b, c = trainModel1.run_getStaticFeature("D:\desktop\PKI大作业\scout-main\check\\1.php")
